...
impl Order {
    fn validate_product_name(prod: &String) {
        if (*prod).is_empty() {
            panic!("Cannot have an empty name for the product!");
        }
    ...
    fn validate_quantity(qn: &u32) {
        if *qn == 0 {
            panic!("Quantity must be greater than zero!");
        }
    }
    fn validate_unit_price(unit: &u32) {
        if *unit == 0 {
            panic!("Unit must be greater than zero!");
        }
    }
    ...
    pub fn total(&self) -> u32 {
        ...
        Self::validate_unit_price(&self.unit_price);
