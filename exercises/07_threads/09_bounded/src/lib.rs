...
pub enum Command {
    Insert {
        draft: TicketDraft,
        response_channel: SyncSender<Result<TicketId, TrySendError<Command>>>,
    },
    Get {
        id: TicketId,
        response_channel: SyncSender<Result<Option<Ticket>, TrySendError<Command>>>,
    },
}
    ...
    loop {
        match receiver.recv() {
            Ok(Command::Insert {
                ...
                let _ = response_channel.try_send(Ok(id));
            }
            Ok(Command::Get {
                id,
                response_channel,
                ...
                let _ = response_channel.try_send(Ok(ticket.cloned()));
            }
            Err(_) => {
